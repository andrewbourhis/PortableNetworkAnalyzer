 ...
 * @author Michael Meli
 *
 * MODIFIED BY ANDREW BOURHIS on 9/20/2017
 */
...
/**
 * Set the number of settling cycles used in sweep
 *
 * @param nCycles Number of settle cycles to be programmed into the AD5933
 * @param multiplier Either 1, 2, or 4
 *
 * D10	D9		Multiplier value
 * 0	0		Default (x1)
 * 0	1		x2
 * 1	1		x4
 *
 * @return Success or failure
 */
bool AD5933::setSettlingCycles(int nCycles, int multiplier) {
    byte highByte, lowByte;
    if(nCycles > 511) return false;
	if(multiplier == 1) highByte = NUM_CYCLES_X1;
	else if(multiplier == 2) highByte = NUM_CYCLES_X2;
    else if(multiplier == 4) highByte = NUM_CYCLES_X4;
    else return false;
    
    highByte |= (nCycles >> 8) & 0xFF;
    lowByte = nCycles & 0xFF;
    
    if(!sendByte(NUM_SCYCLES_1,highByte)) return false;
    return sendByte(NUM_SCYCLES_2,lowByte);
}

/**
 * Set the entire control register in a single exchange.
 *
 * @param reg1 Control register bits 15 to 8
 * @param reg2 Control register bits 7 to 0
 * @return Success or failure
 *
 *
 *	Note that the following states are not allowed for reg1:
 ...
 *	1111XXXX
 *
 *	Additionally, bit D11 is assumed to be 0
 *
 */
 bool AD5933::setControlRegisters(byte reg1, byte reg2){
 	
 	if((reg1 & 0xF0) == (0b01010000) ||
	 	(reg1 & 0xF0) == (0b01100000) ||
		(reg1 & 0xF0) == (0b01110000) ||
		(reg1 & 0xF0) == (0b11100000) ||
		(reg1 & 0xF0) == (0b11110000))
			return false;
	
 	if(!sendByte(CTRL_REG1,reg1))
 		return false;
 	else
 		return sendByte(CTRL_REG2,reg2);
 }
 ...
 * @return Success or failure
 */
bool AD5933::setStartFrequency(unsigned long start, unsigned long clockSpeed) {
